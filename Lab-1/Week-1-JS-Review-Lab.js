...
function realtorFees(listings){
  let price = Number(listings.price) //Converts string to a number
  let priceOfFees = ((listings[i]).price <= 450000)? price * 0.025 : price * 0.02;
  
  return priceOfFees.toString(); //returns the price as a string
}
...
let listingAscendingly = listings.toSorted((a ,b) => Number(a.price) - Number(b.price));
...
let newBuiltListings = listings.filter(listing => listings.builtIn <= 1990);